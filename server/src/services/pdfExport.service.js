...
// Generates a styled PDF summary of a ClearClause analysis using pdfkit.
...
function generateAnalysisPDF(docData, analysis, res) {
  ...
  res.setHeader('Content-Disposition', `attachment; filename="ClearClause_${safeName}.pdf"`);
  doc.pipe(res);
  ...
  doc.text('ClearClause', LEFT, 18, { lineBreak: false });
  ...
  for (let i = 0; i < totalPages; i++) {
    ...
    doc.font('Helvetica').fontSize(8).fillColor(C.muted);
    doc.text(
      `Generated by ClearClause · Page ${i + 1} of ${totalPages} · This report is for informational purposes only and is not legal advice.`,
      LEFT, pageBottom,
